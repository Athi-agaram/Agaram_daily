...
export default function TopBar({ username = "Administrator", userGroup = "Administrator" }) {
  ...
  const menuItems = [
    ...
    { label: "Logout", icon: <LogoutSharpIcon fontSize="small" /> },
  ];
  ...
  return (
    ...
    >
      <Toolbar sx={{ justifyContent: "space-between", minHeight: `${topBarHeight}px`, px: 5 }}>
        {/* Left: Logo + Version */}
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 5 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              <Typography sx={{ fontSize: 10, fontWeight: 550, color: "#52526bff" }}>Time Zone:</Typography>
              <Typography sx={{ fontSize: 10, fontWeight: 500, color: "#000" }}>Asia/Kolkata (UTC+05:30)</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              <Typography sx={{ fontSize: 10, fontWeight: 550, color: "#555" }}>Domain:</Typography>
              <Typography sx={{ fontSize: 10, fontWeight: 500, color: "#000" }}>SDMS</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              <Typography sx={{ fontSize: 10, fontWeight: 550, color: "#555" }}>Site:</Typography>
              <Typography sx={{ fontSize: 10, fontWeight: 500, color: "#000" }}>Mumbai</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
              <Typography sx={{ fontSize: 14, fontWeight: 600, color: "#2b2a2a" }}>{userGroup}</Typography>
              <Typography sx={{ fontSize: 12, fontWeight: 500, color: "#6d6a6aff" }}>{username}</Typography>
            </Box>
                    ...
                    {menuItems.map((item, index) => (
                      <React.Fragment key={index}>
                        ...
                        >
                          ...
                          <Box sx={{ ml: 2, color: "#1565c0" }}>{item.icon}</Box> {/* blue icon */}
                        </MenuItem>
