...
function HomePage() {
  ...
  const menuItems = [
    { text: "Component1", icon: <InboxIcon /> },
    { text: "Component2", icon: <MailIcon /> },
    { text: "Component3", icon: <CloudUploadSharpIcon /> },
  ];
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <AppBar position="fixed" open={open}>
        <Toolbar>
          ...
          <Typography variant="h6" sx={{ flexGrow: 1 }}>Home Page</Typography>
          <IconButton color="inherit" onClick={() => setSettingsOpen(!settingsOpen)}>
            <SettingsIcon />
          </IconButton>
        </Toolbar>
      ...
      {settingsOpen && (
        <Paper sx={{ position: "fixed", top: 64, right: 16, p: 2, width: 200 }} elevation={3}>
          <Typography variant="subtitle1">Username: {username}</Typography>
